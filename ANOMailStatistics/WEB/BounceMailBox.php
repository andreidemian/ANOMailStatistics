<?php
	function BounceMailBox() {

	include('lib/SQLEmailLog.php');
	include('lib/SQLCSV.php');
	$searchType = array('FROM','TO','MTA','STATUS');
	$selectedType = 'null';
...
<!-- Search menu -->
	<div class="SEARCH ">
		<form action="index.php">
			...
			<div class="container pull-left form-inline" style="width: 300px;">
			...
			<div class="pull-right form-inline" style="margin-right: 20px;">
				<input type="submit" name="export_bounces" class="btn-md btn-primary" value="Export to CSV">
			</div>
		</form>
	</div>
...
<?php
	$Export = new SQLCSV();
	if($Export->download($data) == 'yes') { ?>
	<div class="pull-right form-inline" style="margin-right: 20px;">
		<button class="btn-md btn-success"><a href="uploads/bounces.csv" target="popup">Download CSV</a></button>
	</div>
	<?php } ?>
		...
		foreach($rows as &$row) {
			echo
				'<tr>',
					...
					'<td>',$row['from_addr'],'</td>',
					'<td>',$row['to_addr'],'</td>',
					'<td>',$row['reporting-mta'],'</td>',
					'<td>',$row['remote-mta'],'</td>',
					'<td>',$row['status'],'</td>',
					'<td><a href="details.php?id='.$row['id'].'&q_type=BD" target="_blank">Details</a></td>',
				'</tr>';
	}
?>
		</tbody>
