...
class SQLMailLog {
	...
	private function UpdateLog($data) {
		...
		$connect = $this->ConnectToDB();
		$result = $connect->prepare('update config_log set logrotate = ?, log = ?, log_description = ?, logtype = ?, iteration_num = ?, R_H = ?, R_M = ?, R_W = ?, del_older_rows = ?, del_older_logs = ?, active = ? where id = ?');
		$result->bind_param('issiiiiiiiii',$data['logrotate'],$data['log'],$data['log_des'],$logtype,$data['ITN'],$time[0],$time[1],$data['week'],$data['deldb'],$data['dellog'],$data['active'],$data['id']);
		$result->execute();
		$result->close();
		$connect->close();
	}
